...
/* --- Old div-based LogoReveal kept as fallback (unused) --- */
// function LogoReveal({ play }: { play: boolean }) { ... }

/* --- Inline SVG logo with GSAP-driven clip-path polygon animation --- */
const BIRSE_START = "-43,88 -10,0 -10,0 -43,88";
const BIRSE_END   = "-43,88 -10,0 298,0 265,88";
const THOMAS_START = "713,0 680,88 680,88 713,0";
const THOMAS_END   = "258,0 225,88 680,88 713,0";

function lerpPoints(a: string, b: string, t: number) {
  const pa = a.split(/[\s,]+/).map(Number);
  const pb = b.split(/[\s,]+/).map(Number);
  return pa.map((v, i) => v + (pb[i] - v) * t).join(",");
}

function LogoRevealSVG({ play }: { play: boolean }) {
  const birsePolyRef = useRef<SVGPolygonElement>(null);
  const thomasPolyRef = useRef<SVGPolygonElement>(null);
  const slashRef = useRef<SVGGElement>(null);
  const ran = useRef(false);

  useEffect(() => {
    if (!play || ran.current) return;
    const birsePoly = birsePolyRef.current;
    const thomasPoly = thomasPolyRef.current;
    const slash = slashRef.current;
    if (!birsePoly || !thomasPoly || !slash) return;
    ran.current = true;
    ...
    gsap.to(bp, {
      ...
      onUpdate: () => {
        birsePoly.setAttribute("points", lerpPoints(BIRSE_START, BIRSE_END, bp.t));
      },
    });
    ...
    gsap.to(tp, {
      t: 1, duration: dur, ease,
      onUpdate: () => {
        thomasPoly.setAttribute("points", lerpPoints(THOMAS_START, THOMAS_END, tp.t));
      },
    });

    gsap.fromTo(slash, { opacity: 0 }, { opacity: 1, duration: 0.4, delay: 0.8, ease: "power2.inOut" });
  }, [play]);
  ...
  return (
    ...
    >
      <defs>
        <clipPath id="clip0">
          <rect width="668.79" height="87.46" fill="white" />
        </clipPath>
        <clipPath id="birseWipe">
          <polygon ref={birsePolyRef} points={BIRSE_START} />
        </clipPath>
        <clipPath id="thomasWipe">
          <polygon ref={thomasPolyRef} points={THOMAS_START} />
        </clipPath>
      </defs>

      <g clipPath="url(#clip0)">
        {/* birse */}
        <g clipPath="url(#birseWipe)">
    ...
    <>
      ...
      <div className="relative h-dvh w-full overflow-hidden bg-black">
        ...
        <div
          ...
          style={{ filter: "drop-shadow(0 2px 48px rgba(0,0,0,0.45))" }}
        >
